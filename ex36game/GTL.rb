...
def prompt(values)
  ...
  # Option 5 and 6 can only be chosen once.
...
# User only has 6 turns, choice adds a value to values[0]
# time_check triggers the end event when values[0] >= 5.
...
# This is the end event that triggered when values[0] >= 5
def fight(values)
  ...
  if selection == "1"
    ...
    puts "*Drops mic*"
    $stdin.gets
    puts "You walk towards the club entrance."
...
def club(values)
# needs to be made.
# Depending on the status of the array values, the event will have different options and outcomes.
end

# triggered when the players line of events has ended.
